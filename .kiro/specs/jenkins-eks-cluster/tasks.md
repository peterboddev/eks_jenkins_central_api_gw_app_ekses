...
- [-] 1. Set up CDK project structure and core dependencies
...
- [ ] 2. Implement VPC and networking infrastructure
  - [~] 2.1 Create VPC with CIDR 10.0.0.0/16
  ...
  - [~] 2.2 Add NAT Gateways for outbound connectivity
...
- [ ] 3. Implement EKS cluster with IAM roles
  - [~] 3.1 Create EKS cluster IAM role
    - Define IAM role with AmazonEKSClusterPolicy
    - Configure trust relationship for EKS service
    - _Requirements: 1.1_
  
  - [~] 3.2 Create EKS cluster resource
...
- [ ] 4. Implement EFS file system for Jenkins storage
...
- [ ] 5. Implement S3 bucket for artifacts and job state
...
- [ ] 6. Implement IAM roles for IRSA
  - [~] 6.1 Create OIDC provider for EKS cluster
    - Configure OIDC identity provider for the EKS cluster
    - _Requirements: 5.3_
  
  - [~] 6.2 Create Jenkins controller IAM role
...
- [ ] 7. Implement EKS node groups
  - [~] 7.1 Create controller node group (on-demand)
  ...
  - [~] 7.2 Create agent node group (spot)
...
- [ ] 8. Install and configure EFS CSI Driver
  - [~] 8.1 Deploy EFS CSI Driver
...
- [ ] 9. Deploy Jenkins controller
  - [~] 9.1 Create Jenkins namespace and service account
  ...
  - [~] 9.2 Create Jenkins controller StatefulSet
  ...
  - [~] 9.3 Create persistent volume claim for Jenkins home
  ...
  - [~] 9.4 Create Jenkins service
    - Define ClusterIP service
    - Expose port 8080 (HTTP) and port 50000 (JNLP)
    - _Requirements: 3.4_
  
  - [~] 9.5 Configure Jenkins pod restart policy
...
- [ ] 10. Configure Jenkins agent pod template
  - [~] 10.1 Create Jenkins agent pod template configuration
...
- [ ] 11. Deploy Cluster Autoscaler
  - [~] 11.1 Create Cluster Autoscaler deployment
...
- [ ] 12. Deploy AWS Node Termination Handler
  - [~] 12.1 Create Node Termination Handler DaemonSet
...
- [ ] 14. Configure monitoring and observability
  - [~] 14.1 Enable CloudWatch Container Insights
  ...
  - [ ]* 14.3 Create monitoring dashboards
...
- [ ] 15. Write property-based tests
  - [~] 15.1 Write property test for data persistence across pod restarts
  ...
  - [~] 15.2 Write property test for persistent volume remounting
...
- [ ] 17. Create deployment documentation
  ...
  - [ ]* 17.2 Document recovery procedures
